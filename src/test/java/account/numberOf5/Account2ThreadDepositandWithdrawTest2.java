...
public class Account2ThreadDepositandWithdrawTest2 extends TestCase{

	Account2ThreadDepositandWithdrawTest2 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest2();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",9605);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8136));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,4033));
		}
