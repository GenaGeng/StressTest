...
public class Account2ThreadDepositandWithdrawTest77 extends TestCase{

	Account2ThreadDepositandWithdrawTest77 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest77();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5477);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,10));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1236));
		}
