...
public class Account2ThreadDepositandWithdrawTest84 extends TestCase{

	Account2ThreadDepositandWithdrawTest84 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest84();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",7608);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6918));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3077));
		}
