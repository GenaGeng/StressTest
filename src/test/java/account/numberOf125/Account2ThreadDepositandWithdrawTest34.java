...
public class Account2ThreadDepositandWithdrawTest34 extends TestCase{

	Account2ThreadDepositandWithdrawTest34 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest34();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5756);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,3360));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,4282));
		}
