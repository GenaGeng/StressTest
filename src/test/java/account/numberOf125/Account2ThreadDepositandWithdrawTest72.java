...
public class Account2ThreadDepositandWithdrawTest72 extends TestCase{

	Account2ThreadDepositandWithdrawTest72 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest72();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5785);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,814));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2364));
		}
