...
public class Account2ThreadDepositandWithdrawTest8 extends TestCase{

	Account2ThreadDepositandWithdrawTest8 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest8();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6816);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,3101));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,986));
		}
