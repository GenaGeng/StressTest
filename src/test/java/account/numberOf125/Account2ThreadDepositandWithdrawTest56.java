...
public class Account2ThreadDepositandWithdrawTest56 extends TestCase{

	Account2ThreadDepositandWithdrawTest56 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest56();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6891);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,5977));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,729));
		}
