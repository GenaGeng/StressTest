...
public class Account2ThreadDepositandWithdrawTest60 extends TestCase{

	Account2ThreadDepositandWithdrawTest60 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest60();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3521);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2387));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,693));
		}
