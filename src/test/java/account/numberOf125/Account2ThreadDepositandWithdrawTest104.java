...
public class Account2ThreadDepositandWithdrawTest104 extends TestCase{

	Account2ThreadDepositandWithdrawTest104 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest104();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",7803);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6164));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,7489));
		}
