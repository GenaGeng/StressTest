...
public class Account2ThreadDepositandWithdrawTest6 extends TestCase{

	Account2ThreadDepositandWithdrawTest6 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest6();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3561);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,7034));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2918));
		}
