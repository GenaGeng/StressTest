...
public class Account2ThreadDepositandWithdrawTest32 extends TestCase{

	Account2ThreadDepositandWithdrawTest32 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest32();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5470);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,7096));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,5057));
		}
