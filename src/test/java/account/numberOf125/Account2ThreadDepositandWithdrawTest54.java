...
public class Account2ThreadDepositandWithdrawTest54 extends TestCase{

	Account2ThreadDepositandWithdrawTest54 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest54();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",1384);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,698));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,661));
		}
