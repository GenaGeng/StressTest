...
public class Account2ThreadDepositandWithdrawTest52 extends TestCase{

	Account2ThreadDepositandWithdrawTest52 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest52();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",8215);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,9294));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3177));
		}
