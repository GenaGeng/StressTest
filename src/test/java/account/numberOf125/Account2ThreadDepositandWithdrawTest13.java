...
public class Account2ThreadDepositandWithdrawTest13 extends TestCase{

	Account2ThreadDepositandWithdrawTest13 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest13();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",741);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,9243));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,191));
		}
