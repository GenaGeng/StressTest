...
public class Account2ThreadDepositandWithdrawTest24 extends TestCase{

	Account2ThreadDepositandWithdrawTest24 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest24();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",1957);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,602));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,954));
		}
