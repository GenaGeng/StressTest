...
public class Account2ThreadDepositandWithdrawTest91 extends TestCase{

	Account2ThreadDepositandWithdrawTest91 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest91();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",7314);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,9162));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,598));
		}
