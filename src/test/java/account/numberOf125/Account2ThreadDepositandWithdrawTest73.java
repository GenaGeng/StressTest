...
public class Account2ThreadDepositandWithdrawTest73 extends TestCase{

	Account2ThreadDepositandWithdrawTest73 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest73();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6760);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2135));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2163));
		}
