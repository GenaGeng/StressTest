...
public class Account2ThreadDepositandWithdrawTest16 extends TestCase{

	Account2ThreadDepositandWithdrawTest16 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest16();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",8636);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,5550));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,6913));
		}
