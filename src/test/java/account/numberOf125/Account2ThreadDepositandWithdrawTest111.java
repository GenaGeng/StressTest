...
public class Account2ThreadDepositandWithdrawTest111 extends TestCase{

	Account2ThreadDepositandWithdrawTest111 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest111();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3969);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,653));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3830));
		}
