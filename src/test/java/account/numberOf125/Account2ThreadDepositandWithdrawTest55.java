...
public class Account2ThreadDepositandWithdrawTest55 extends TestCase{

	Account2ThreadDepositandWithdrawTest55 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest55();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",2521);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,7813));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2445));
		}
