...
public class Account2ThreadDepositandWithdrawTest76 extends TestCase{

	Account2ThreadDepositandWithdrawTest76 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest76();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",1805);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,5980));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,112));
		}
