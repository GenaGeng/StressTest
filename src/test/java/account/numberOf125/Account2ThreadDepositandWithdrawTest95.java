...
public class Account2ThreadDepositandWithdrawTest95 extends TestCase{

	Account2ThreadDepositandWithdrawTest95 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest95();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5590);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,4823));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,559));
		}
