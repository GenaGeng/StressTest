...
public class Account2ThreadDepositandWithdrawTest87 extends TestCase{

	Account2ThreadDepositandWithdrawTest87 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest87();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3718);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2685));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3247));
		}
