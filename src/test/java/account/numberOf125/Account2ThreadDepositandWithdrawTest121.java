...
public class Account2ThreadDepositandWithdrawTest121 extends TestCase{

	Account2ThreadDepositandWithdrawTest121 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest121();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",1862);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,3036));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,65));
		}
