...
public class Account2ThreadDepositandWithdrawTest64 extends TestCase{

	Account2ThreadDepositandWithdrawTest64 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest64();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",1422);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,548));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,182));
		}
