...
public class Account2ThreadDepositandWithdrawTest89 extends TestCase{

	Account2ThreadDepositandWithdrawTest89 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest89();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6051);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8634));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1435));
		}
