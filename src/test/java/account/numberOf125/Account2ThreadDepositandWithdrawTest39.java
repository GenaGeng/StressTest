...
public class Account2ThreadDepositandWithdrawTest39 extends TestCase{

	Account2ThreadDepositandWithdrawTest39 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest39();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",8788);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6700));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,7322));
		}
