...
public class Account2ThreadDepositandWithdrawTest119 extends TestCase{

	Account2ThreadDepositandWithdrawTest119 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest119();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",7996);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2898));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,7305));
		}
