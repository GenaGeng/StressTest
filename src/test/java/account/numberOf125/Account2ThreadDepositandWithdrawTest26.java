...
public class Account2ThreadDepositandWithdrawTest26 extends TestCase{

	Account2ThreadDepositandWithdrawTest26 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest26();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",8950);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2931));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3644));
		}
