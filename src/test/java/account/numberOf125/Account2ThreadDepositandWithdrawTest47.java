...
public class Account2ThreadDepositandWithdrawTest47 extends TestCase{

	Account2ThreadDepositandWithdrawTest47 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest47();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",7665);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,5398));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2195));
		}
