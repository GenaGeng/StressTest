...
public class Account2ThreadDepositandWithdrawTest83 extends TestCase{

	Account2ThreadDepositandWithdrawTest83 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest83();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5357);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6818));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,5219));
		}
