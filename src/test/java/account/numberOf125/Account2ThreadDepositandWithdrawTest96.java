...
public class Account2ThreadDepositandWithdrawTest96 extends TestCase{

	Account2ThreadDepositandWithdrawTest96 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest96();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3942);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,1109));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1296));
		}
