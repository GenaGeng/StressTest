...
public class Account2ThreadDepositandWithdrawTest120 extends TestCase{

	Account2ThreadDepositandWithdrawTest120 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest120();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",750);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,1879));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,287));
		}
