...
public class Account2ThreadDepositandWithdrawTest50 extends TestCase{

	Account2ThreadDepositandWithdrawTest50 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest50();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",4175);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8203));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2572));
		}
