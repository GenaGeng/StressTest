...
public class Account2ThreadDepositandWithdrawTest113 extends TestCase{

	Account2ThreadDepositandWithdrawTest113 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest113();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3419);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,4322));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1162));
		}
