...
public class Account2ThreadDepositandWithdrawTest112 extends TestCase{

	Account2ThreadDepositandWithdrawTest112 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest112();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3726);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2992));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,865));
		}
