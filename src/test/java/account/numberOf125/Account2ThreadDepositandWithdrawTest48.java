...
public class Account2ThreadDepositandWithdrawTest48 extends TestCase{

	Account2ThreadDepositandWithdrawTest48 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest48();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6206);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,3276));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,336));
		}
