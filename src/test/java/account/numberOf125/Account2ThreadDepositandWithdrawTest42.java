...
public class Account2ThreadDepositandWithdrawTest42 extends TestCase{

	Account2ThreadDepositandWithdrawTest42 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest42();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3653);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,202));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1315));
		}
