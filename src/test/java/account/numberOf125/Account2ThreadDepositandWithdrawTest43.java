...
public class Account2ThreadDepositandWithdrawTest43 extends TestCase{

	Account2ThreadDepositandWithdrawTest43 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest43();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",7808);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8547));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,6185));
		}
