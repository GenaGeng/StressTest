...
public class Account2ThreadDepositandWithdrawTest75 extends TestCase{

	Account2ThreadDepositandWithdrawTest75 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest75();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3052);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,1917));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2178));
		}
