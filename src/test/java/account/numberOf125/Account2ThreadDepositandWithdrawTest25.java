...
public class Account2ThreadDepositandWithdrawTest25 extends TestCase{

	Account2ThreadDepositandWithdrawTest25 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest25();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6394);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8916));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2157));
		}
