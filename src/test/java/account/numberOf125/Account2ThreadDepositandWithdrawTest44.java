...
public class Account2ThreadDepositandWithdrawTest44 extends TestCase{

	Account2ThreadDepositandWithdrawTest44 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest44();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",4041);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,4533));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1643));
		}
