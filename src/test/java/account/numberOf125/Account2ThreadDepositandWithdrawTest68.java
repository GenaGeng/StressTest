...
public class Account2ThreadDepositandWithdrawTest68 extends TestCase{

	Account2ThreadDepositandWithdrawTest68 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest68();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",126);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,5338));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,0));
		}
