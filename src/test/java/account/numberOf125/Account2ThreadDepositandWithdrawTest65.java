...
public class Account2ThreadDepositandWithdrawTest65 extends TestCase{

	Account2ThreadDepositandWithdrawTest65 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest65();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",272);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,9640));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,148));
		}
