...
public class Account2ThreadDepositandWithdrawTest102 extends TestCase{

	Account2ThreadDepositandWithdrawTest102 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest102();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6468);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,1453));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,4671));
		}
