...
public class Account2ThreadDepositandWithdrawTest46 extends TestCase{

	Account2ThreadDepositandWithdrawTest46 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest46();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",8242);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,9147));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,251));
		}
