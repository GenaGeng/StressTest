...
public class Account2ThreadDepositandWithdrawTest85 extends TestCase{

	Account2ThreadDepositandWithdrawTest85 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest85();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",4803);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2845));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,7));
		}
