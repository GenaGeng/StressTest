...
public class Account2ThreadDepositandWithdrawTest61 extends TestCase{

	Account2ThreadDepositandWithdrawTest61 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest61();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3047);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,975));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2219));
		}
