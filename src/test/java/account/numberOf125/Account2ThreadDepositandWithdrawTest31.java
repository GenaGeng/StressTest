...
public class Account2ThreadDepositandWithdrawTest31 extends TestCase{

	Account2ThreadDepositandWithdrawTest31 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest31();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5561);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,9151));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1988));
		}
