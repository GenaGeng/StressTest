...
public class Account2ThreadDepositandWithdrawTest81 extends TestCase{

	Account2ThreadDepositandWithdrawTest81 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest81();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",2881);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,7574));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2558));
		}
