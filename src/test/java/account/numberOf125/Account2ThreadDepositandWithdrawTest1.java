...
public class Account2ThreadDepositandWithdrawTest1 extends TestCase{

	Account2ThreadDepositandWithdrawTest1 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest1();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5607);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,5868));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2981));
		}
