...
public class Account2ThreadDepositandWithdrawTest69 extends TestCase{

	Account2ThreadDepositandWithdrawTest69 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest69();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",9806);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,425));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,5031));
		}
