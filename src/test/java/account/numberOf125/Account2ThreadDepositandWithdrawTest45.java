...
public class Account2ThreadDepositandWithdrawTest45 extends TestCase{

	Account2ThreadDepositandWithdrawTest45 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest45();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",4744);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2784));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2197));
		}
