...
public class Account2ThreadDepositandWithdrawTest41 extends TestCase{

	Account2ThreadDepositandWithdrawTest41 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest41();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",4257);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8524));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2793));
		}
