...
public class Account2ThreadDepositandWithdrawTest106 extends TestCase{

	Account2ThreadDepositandWithdrawTest106 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest106();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",9712);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6967));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,6201));
		}
