...
public class Account2ThreadDepositandWithdrawTest88 extends TestCase{

	Account2ThreadDepositandWithdrawTest88 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest88();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",9466);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8473));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,183));
		}
