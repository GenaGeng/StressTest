...
public class Account2ThreadDepositandWithdrawTest82 extends TestCase{

	Account2ThreadDepositandWithdrawTest82 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest82();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",8567);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8308));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,5913));
		}
