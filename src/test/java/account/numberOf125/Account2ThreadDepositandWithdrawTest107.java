...
public class Account2ThreadDepositandWithdrawTest107 extends TestCase{

	Account2ThreadDepositandWithdrawTest107 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest107();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5313);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8173));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3322));
		}
