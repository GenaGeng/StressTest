...
public class Account2ThreadDepositandWithdrawTest62 extends TestCase{

	Account2ThreadDepositandWithdrawTest62 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest62();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",9771);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,1199));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1143));
		}
