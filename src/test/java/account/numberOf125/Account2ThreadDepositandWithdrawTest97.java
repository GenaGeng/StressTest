...
public class Account2ThreadDepositandWithdrawTest97 extends TestCase{

	Account2ThreadDepositandWithdrawTest97 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest97();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",7688);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,3810));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1843));
		}
