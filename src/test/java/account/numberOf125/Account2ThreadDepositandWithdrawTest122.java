...
public class Account2ThreadDepositandWithdrawTest122 extends TestCase{

	Account2ThreadDepositandWithdrawTest122 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest122();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5647);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2418));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,810));
		}
