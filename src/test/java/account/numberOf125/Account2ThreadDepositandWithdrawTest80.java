...
public class Account2ThreadDepositandWithdrawTest80 extends TestCase{

	Account2ThreadDepositandWithdrawTest80 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest80();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6443);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,137));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3983));
		}
