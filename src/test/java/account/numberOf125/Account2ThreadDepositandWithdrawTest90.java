...
public class Account2ThreadDepositandWithdrawTest90 extends TestCase{

	Account2ThreadDepositandWithdrawTest90 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest90();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",1691);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6542));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,381));
		}
