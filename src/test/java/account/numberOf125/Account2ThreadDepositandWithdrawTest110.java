...
public class Account2ThreadDepositandWithdrawTest110 extends TestCase{

	Account2ThreadDepositandWithdrawTest110 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest110();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",5021);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,5464));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3303));
		}
