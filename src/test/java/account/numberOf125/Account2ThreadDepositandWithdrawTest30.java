...
public class Account2ThreadDepositandWithdrawTest30 extends TestCase{

	Account2ThreadDepositandWithdrawTest30 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest30();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3527);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,4703));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2668));
		}
