...
public class Account2ThreadDepositandWithdrawTest37 extends TestCase{

	Account2ThreadDepositandWithdrawTest37 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest37();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6045);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,1515));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1086));
		}
