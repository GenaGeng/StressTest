...
public class Account2ThreadDepositandWithdrawTest114 extends TestCase{

	Account2ThreadDepositandWithdrawTest114 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest114();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",1329);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,357));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,375));
		}
