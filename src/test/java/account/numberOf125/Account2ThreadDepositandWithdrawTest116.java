...
public class Account2ThreadDepositandWithdrawTest116 extends TestCase{

	Account2ThreadDepositandWithdrawTest116 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest116();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",2596);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,3397));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,795));
		}
