...
public class Account2ThreadDepositandWithdrawTest28 extends TestCase{

	Account2ThreadDepositandWithdrawTest28 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest28();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",6684);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,1777));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,585));
		}
