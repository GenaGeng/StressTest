...
public class Account2ThreadDepositandWithdrawTest36 extends TestCase{

	Account2ThreadDepositandWithdrawTest36 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest36();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",4779);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,8210));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2105));
		}
