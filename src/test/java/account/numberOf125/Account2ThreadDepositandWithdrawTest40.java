...
public class Account2ThreadDepositandWithdrawTest40 extends TestCase{

	Account2ThreadDepositandWithdrawTest40 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest40();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",8367);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,4624));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,8215));
		}
