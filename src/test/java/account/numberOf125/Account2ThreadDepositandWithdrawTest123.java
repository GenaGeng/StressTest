...
public class Account2ThreadDepositandWithdrawTest123 extends TestCase{

	Account2ThreadDepositandWithdrawTest123 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest123();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",9104);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2208));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,6112));
		}
