...
public class Account2ThreadDepositandWithdrawTest92 extends TestCase{

	Account2ThreadDepositandWithdrawTest92 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest92();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",468);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,1283));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,244));
		}
