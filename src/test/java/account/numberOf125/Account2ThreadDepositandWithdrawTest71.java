...
public class Account2ThreadDepositandWithdrawTest71 extends TestCase{

	Account2ThreadDepositandWithdrawTest71 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest71();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",2969);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,7567));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2813));
		}
