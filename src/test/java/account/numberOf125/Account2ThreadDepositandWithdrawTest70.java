...
public class Account2ThreadDepositandWithdrawTest70 extends TestCase{

	Account2ThreadDepositandWithdrawTest70 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest70();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",2562);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,2647));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1079));
		}
