...
public class Account2ThreadDepositandWithdrawTest19 extends TestCase{

	Account2ThreadDepositandWithdrawTest19 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest19();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",2537);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,5345));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1543));
		}
