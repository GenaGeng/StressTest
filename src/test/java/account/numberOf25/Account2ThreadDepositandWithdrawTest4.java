...
public class Account2ThreadDepositandWithdrawTest4 extends TestCase{

	Account2ThreadDepositandWithdrawTest4 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest4();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",1749);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6569));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1507));
		}
