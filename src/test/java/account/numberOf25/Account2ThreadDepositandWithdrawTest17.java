...
public class Account2ThreadDepositandWithdrawTest17 extends TestCase{

	Account2ThreadDepositandWithdrawTest17 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest17();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",4501);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,7357));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,3692));
		}
