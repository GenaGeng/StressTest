...
public class Account2ThreadDepositandWithdrawTest9 extends TestCase{

	Account2ThreadDepositandWithdrawTest9 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest9();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",2675);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6959));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,2273));
		}
