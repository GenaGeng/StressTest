...
public class Account2ThreadDepositandWithdrawTest22 extends TestCase{

	Account2ThreadDepositandWithdrawTest22 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest22();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",8684);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,4598));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,5476));
		}
