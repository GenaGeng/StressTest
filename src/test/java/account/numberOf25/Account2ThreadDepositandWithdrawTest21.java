...
public class Account2ThreadDepositandWithdrawTest21 extends TestCase{

	Account2ThreadDepositandWithdrawTest21 accountTest;

	@Before
	public void setUp(){

		accountTest = new Account2ThreadDepositandWithdrawTest21();
	}
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",3601);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,6151));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,1735));
		}
