...
public class Account2ThreadDepositandWithdrawTest24 extends TestCase{
	...
	public boolean performDepositsAndWithdrawalsAndCheckInvariant(int numThreads) throws Exception {
		Account account = new Account("The Account",529);
		Thread[] withdrawalThreads = new Thread[numThreads];

		Thread[] depositThreads = new Thread[numThreads];

		for (int i = 0; i < numThreads; i++) {
			depositThreads[i] = new Thread(new DepositThread(account,3859));
			withdrawalThreads[i] = new Thread(new WithdrawThread(account,59));
		}
